...
async function uploadToImgBB(file: File): Promise<string> {
  if (!IMGBB_API_KEY) {
    throw new Error('ImgBB API key not set. Add NEXT_PUBLIC_IMGBB_API_KEY to your .env.local file.')
  }
  ...
  return (
    <div className="p-8 max-w-2xl">
      ...
      <div className="space-y-4">
        ...
        {/* ── Base URL — most important setting for QR codes to work on phones ── */}
        <div className="bg-orange-50 rounded-xl p-5 border border-orange-200 shadow-sm">
          <label className="block text-[10px] font-bold text-orange-400 uppercase tracking-widest mb-2 flex items-center gap-2">
            ...
            System Base URL (Local IP) — Required for QR codes
          </label>
          <input
          ...
          {/* CHANGE 1: Live QR preview so you can confirm the URL right here */}
          {settings.baseUrl && (
          ...
          <p className="text-[10px] text-orange-600 mt-3 italic font-medium leading-relaxed">
            * This must be your computer&apos;s local IP (e.g. <strong>http://192.168.1.42:3000</strong>), not
            localhost. Both this machine and customer phones must be on the same Wi-Fi network.
...
export default function AdminPage() {
  ...
  const topTable = orders.length > 0
    ? orders.reduce((acc, o) => { acc[o.tableNumber] = (acc[o.tableNumber] || 0) + 1; return acc }, {} as Record<number, number>)
    : {}
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setImageUploadStatus('error')
      setImageUploadMsg(
        `ImgBB upload failed: ${error.message}. ` +
        `The preview is temporary — save will use this session URL only. ` +
        `Check your NEXT_PUBLIC_IMGBB_API_KEY in .env.local.`
      )
    }
  }
  ...
  const handleCreateTable = async () => {
    ...
    if (!newTable.tableNumber || newTable.tableNumber <= 0) {
      setCreateTableError('Please enter a valid table number.')
      return
    }
    if (!newTable.name.trim()) {
      setCreateTableError('Please enter a table name.')
      return
    }
    if (tables.some(t => t.tableNumber === newTable.tableNumber)) {
      setCreateTableError(`Table #${newTable.tableNumber} already exists.`)
      return
    }
    const tableId = await createTable({
      ...
      qrCode: `table-${newTable.tableNumber}`,
    ...
    if (tableId) {
      ...
      setNewTable({ tableNumber: 0, name: '', capacity: 4, shape: 'square' })
      setCreateTableError(null)
    } else {
  ...
  return (
    <>
      <div className="bg-background min-h-screen font-sans">
        {mobileSidebar && (
          <div className="fixed inset-0 bg-black/40 z-[55] md:hidden" onClick={() => setMobileSidebar(false)} />
        )}
            ...
            {([
              ...
              <button
                ...
                className={`flex items-center gap-3 w-full px-4 py-3 text-sm font-medium transition-all rounded-lg ${
                  view === item.key ? 'text-primary bg-orange-50' : 'text-zinc-600 hover:bg-zinc-200 hover:text-zinc-900'
                }`}
              >
            ...
            <div className="p-8 max-w-7xl">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={() => { setCreateTableError(null); setShowCreateTableModal(true) }}
                    className="bg-primary hover:bg-orange-800 text-white px-6 py-3 rounded-xl flex items-center gap-2 font-bold transition-all shadow-lg active:scale-95"
                  >
                    <span className="material-symbols-outlined">add</span>
                    CREATE TABLE
              ...
              {/* CHANGE 2: Warning banner when baseUrl is not set */}
              {!settings.baseUrl && (
                <div className="mb-6 bg-amber-50 border border-amber-300 rounded-xl px-5 py-4 flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-amber-700 mt-1">
                      ...
                      set the Base URL to your computer&apos;s local IP address (e.g. <code className="font-mono bg-amber-100 px-1 rounded">http://192.168.1.42:3000</code>).
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
                {[
                  ...
                  <div key={stat.label} className="bg-white p-6 rounded-xl shadow-sm border border-zinc-100 flex flex-col justify-between">
                    <div className="flex justify-between items-start">
              ...
              {tables.length === 0 ? (
                <div className="bg-white rounded-2xl p-12 text-center border border-zinc-200 shadow-sm">
                  ...
                  <p className="text-zinc-500 mb-6">Create your first table to start generating QR codes for customers.</p>
                  <button
                    onClick={() => { setCreateTableError(null); setShowCreateTableModal(true) }}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {tables.map(table => {
                    // CHANGE 3: Always prefer settings.baseUrl; only fall back to origin
                    // as last resort so the URL shown in the card is always scannable
                    ...
                    return (
                      <div key={table.id} className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden hover:shadow-xl transition-all">
                        <div className="p-6">
                          ...
                          {/* CHANGE 4: Show the exact URL the QR encodes so it's easy to spot issues */}
                          <p className="text-[10px] font-mono text-zinc-400 text-center mb-3 break-all leading-snug px-1">
                            {url}
                          </p>

                          <div className="flex gap-2">
                            <button
                              onClick={() => setPrintTable(table.tableNumber)}
        ...
        {showModal && editingItem && (
          <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl overflow-y-auto max-h-[90vh]">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-2xl font-bold text-zinc-900">
                  {menuItems.find(m => m.id === editingItem.id) ? 'Edit Dish' : 'New Dish'}
                </h3>
              ...
              <div className="space-y-5">
                ...
                <div>
                  ...
                  {editingItem.image && (
                    <div className="mb-3 relative w-full h-36 rounded-xl overflow-hidden border border-zinc-200 bg-zinc-50">
                      <img
                      ...
                      {imageUploadStatus === 'uploading' && (
                        <div className="absolute inset-0 bg-black/50 flex flex-col items-center justify-center gap-2 text-white text-xs font-bold">
                  ...
                  {imageUploadMsg && (
                    <p className={`text-[11px] font-medium mb-3 leading-relaxed px-1 ${
                      imageUploadStatus === 'done' ? 'text-emerald-600' :
                      imageUploadStatus === 'error' ? 'text-red-500' :
                      'text-zinc-400'
                    }`}>
                      {imageUploadMsg}
                    </p>
                  ...
                  <label className={`flex items-center gap-3 w-full bg-zinc-50 border-2 border-dashed rounded-xl px-4 py-3 cursor-pointer transition-colors group ${
                    imageUploadStatus === 'uploading' ? 'border-zinc-200 opacity-60 pointer-events-none' : 'border-zinc-200 hover:border-primary hover:bg-orange-50'
                  }`}>
                    <span className={`material-symbols-outlined text-[22px] transition-colors ${imageUploadStatus === 'uploading' ? 'text-zinc-300' : 'text-zinc-400 group-hover:text-primary'}`}>
                    ...
                    <input
                      type="file"
                      accept="image/*"
                      onChange={e => { resetImageState(); handleImageUpload(e) }}
                  ...
                  <input
                    className="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-2.5 text-zinc-900 focus:ring-2 focus:ring-primary outline-none text-xs font-medium"
                ...
                <button
                  ...
                  disabled={imageUploadStatus === 'uploading'}
                  className="w-full bg-primary text-white py-4 rounded-2xl font-bold active:scale-95 transition-all shadow-lg hover:bg-orange-800 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {imageUploadStatus === 'uploading' ? (
              ...
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={e => setNewTable({ ...newTable, shape: e.target.value as 'square' | 'round' })}
                  >
                ...
                {/* CHANGE 5: Show what URL will be generated for this table number */}
